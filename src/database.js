...
// Garantindo que usamos apenas a instância centralizada do firebase.js
import { db, dbSet, dbPush, dbUpdate, dbRemove, ref, onValue, query, orderByChild, equalTo, getSafeRef } from './firebase.js';
...
export const getEmpresas = (userId, callback) => {
  const empresasRef = getSafeRef('empresas');
...
export const getPeriodos = (empresaId, callback) => {
  return onValue(getSafeRef(`periodos/${empresaId}`), (snapshot) => {
    const periodos = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getAreas = (empresaId, callback) => {
  return onValue(getSafeRef(`areas/${empresaId}`), (snapshot) => {
    const areas = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getResponsaveis = (empresaId, callback) => {
  return onValue(getSafeRef(`responsaveis/${empresaId}`), (snapshot) => {
    const responsaveis = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getEtapas = (empresaId, periodoId, callback) => {
  return onValue(getSafeRef(`etapas/${empresaId}/${periodoId}`), (snapshot) => {
    const etapas = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getHistorico = (empresaId, callback) => {
  return onValue(getSafeRef(`historico/${empresaId}`), (snapshot) => {
    const historico = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getTemplates = (empresaId, callback) => {
  return onValue(getSafeRef(`templates/${empresaId}`), (snapshot) => {
    const templates = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getUsuarios = (callback) => {
  return onValue(getSafeRef('usuarios'), (snapshot) => {
    const usuarios = [];
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
...
export const getUsuario = (uid, callback) => {
  return onValue(getSafeRef(`usuarios/${uid}`), (snapshot) => {
