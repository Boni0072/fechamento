...
import { getAuth as getAuthTool, createUserWithEmailAndPassword, sendPasswordResetEmail, signOut } from 'firebase/auth';
...
const GerenciarUsuarios = () => {
  ...
  useEffect(() => {
    if (!permissaoLoading && !loadingProfile && empresaAtual?.id) { // Só buscar usuários se estiver autorizado e com empresa
      ...
      const usuariosRef = collection(firestore, 'tenants', empresaAtual.id, 'usuarios');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modoEdicao) {
        ...
        const secondaryApp = initializeApp(appInstance.options, 'SecondaryApp');
  ...
  const handleExcluir = async (uid) => {
    ...
    if (window.confirm('Tem certeza que deseja excluir este usuário?') && empresaAtual?.id) {
      try {
        ...
        const usuarioRef = doc(firestore, 'tenants', empresaAtual.id, 'usuarios', uid);
